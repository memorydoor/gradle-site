...
class SiteGenerationTask extends ConventionTask {
    ...
    void copyStyleSheet() {
        copyStyleSheet("maven-base.css")
        copyStyleSheet("maven-theme.css")
    }

    void copyStyleSheet(fileName) {
        File cssFile = new File(getTargetDir(), fileName);
        OutputStream outputStream = new FileOutputStream(cssFile);
        try {
            InputStream cssResource = getClass().getResourceAsStream("/" + fileName);
            try {
